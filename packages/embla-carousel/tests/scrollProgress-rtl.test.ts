...
describe('➡️  ScrollProgress - Horizontal RTL', () => {
  describe('Корректен для каждой точки привязки, когда:', () => {
    test('Слайды БЕЗ ОТСТУПОВ', () => {
      const emblaApi = useEmblaCarousel(mockTestElements(FIXTURE_SCROLL_PROGRESS_RTL_1))
      emblaApi.initOrUpdate({ options: { direction: 'rtl' } })

      expect(emblaApi.scrollProgress()).toBe(-0)

      emblaApi.initOrUpdate({ options: { startIndex: 1 } })
      ...
      emblaApi.initOrUpdate({ options: { startIndex: 3 } })
      expect(emblaApi.scrollProgress()).toBe(0.9583333333333334)

      emblaApi.initOrUpdate({ options: { startIndex: 4 } })
      expect(emblaApi.scrollProgress()).toBe(1)
    })

    test('Слайды С ОТСТУПАМИ', () => {
      const emblaApi = useEmblaCarousel(mockTestElements(FIXTURE_SCROLL_PROGRESS_RTL_2))
      emblaApi.initOrUpdate({ options: { direction: 'rtl' } })
