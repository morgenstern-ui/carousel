...
describe('➡️  ScrollProgress - Vertical', () => {
  describe('Корректен для каждой точки привязки, когда:', () => {
    test('Слайды БЕЗ ОТСТУПОВ', () => {
      const emblaApi = useEmblaCarousel(mockTestElements(FIXTURE_SCROLL_PROGRESS_Y_1))
    ...
    test('Слайды С ОТСТУПАМИ', () => {
      const emblaApi = useEmblaCarousel(mockTestElements(FIXTURE_SCROLL_PROGRESS_Y_2))
      emblaApi.initOrUpdate({ options: { axis: 'y' } })
